...
def test_parse():
    entry = pem.parse(Path("./erdpy_wallet/testdata/alice.pem"), index=0)
    ...
    entry = pem.parse(Path("./erdpy_wallet/testdata/multipleUserKeys.pem"), index=0)
    assert entry.label == "erd1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssycr6th"
    assert entry.message[0:USER_SEED_LENGTH].hex() == "413f42575f7f26fad3317a778771212fdb80245850981e48b58a4f25e344e8f9"

    entry = pem.parse(Path("./erdpy_wallet/testdata/multipleUserKeys.pem"), index=1)
    ...
    entry = pem.parse(Path("./erdpy_wallet/testdata/multipleUserKeys.pem"), index=2)
...
def test_parse_all():
    pairs = pem.parse_all(Path("./erdpy_wallet/testdata/alice.pem"))
    assert len(pairs) == 1
    ...
    pairs = pem.parse_all(Path("./erdpy_wallet/testdata/multipleUserKeys.pem"))
    assert len(pairs) == 3
...
def test_parse_for_validators():
    entry = pem.parse(Path("./erdpy_wallet/testdata/validatorKey00.pem"), index=0)
...
def test_parse_for_validators_all():
    entries = pem.parse_all(Path("./erdpy_wallet/testdata/multipleValidatorKeys.pem"))
