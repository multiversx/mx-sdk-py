...
class CodecForSmallInt:
    def encode_nested_unsigned(self, writer: io.BytesIO, value: int, num_bytes: int):
        data = value.to_bytes(num_bytes, byteorder="big", signed=False)
        writer.write(data)

    def encode_nested_signed(self, writer: io.BytesIO, value: int, num_bytes: int):
    ...
    def decode_nested_unsigned(self, reader: io.BytesIO, value: INumericalValue, num_bytes: int):
        data = read_bytes_exactly(reader, num_bytes)
        value.value = int.from_bytes(data, byteorder="big", signed=False)

    def decode_nested_signed(self, reader: io.BytesIO, value: INumericalValue, num_bytes: int):
        data = read_bytes_exactly(reader, num_bytes)
        value.value = int.from_bytes(data, byteorder="big", signed=True)

    def decode_top_level_unsigned(self, data: bytes, value: INumericalValue, num_bytes: int):
        value.value = int.from_bytes(data, byteorder="big", signed=False)
